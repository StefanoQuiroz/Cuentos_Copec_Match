...
const lenguaje = urlParams.get('lenguaje');//https://____/lenguaje
//let lenguaje = 'EN';
let IDIOMA;
try{
...
//gestiona todas las instancias del reproductor //ANIM.main_tl subtitulos, texto hablado y efectos de sonido
let Player = {
    escena_actual:0,
    parrafo_actual:0,
    audios_parrafo:[],
...
//        console.log("SETEANDO ESCENA");
    ...
    playSoundFX: (cual, loop = false) => {
        let sound;
        if(loop){
            sound = createjs.Sound.play(cual, {interrupt: createjs.Sound.INTERRUPT_ANY, loop:-1});//reproducir un archivo de audio
        } else {
    ...
    cambiaVolume:(cual = 'musica_', vol = 0.1) => {
        ...
        try{   
            let miAudio = todasLasVeces[todasLasVeces.length-1].sound;//posicion exacta de ese audio
            miAudio.volume = vol;
        } catch(err){
    ...
    tooglePlayPause:() => {
        if(ANIM.main_tl.paused()){
            let label = ANIM.main_tl.getLabelBefore();//devuelve la etiqueta anterior que aparece antes del parametro de tiempo
...
//        console.log(cual,Player.senasVisible);
    ...
    pauseVideo:()=>{
        let cual = Player.escena_actual;//numero inicial 0
        if(Player.conVideo){
            try{
                let vid = document.getElementById(`video_${cual}`);
                vid.pause();
...
//Gestiona partes visuales del player
let App = {
    init:function(){
        let w, h;
            w = window.innerWidth;
...
//Gestiona eventos de click y swipe
let Tutorial = {
    slideActual:0,
    init:() => {
        App.init();
